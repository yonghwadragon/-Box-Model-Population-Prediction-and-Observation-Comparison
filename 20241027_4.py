...
# ======================================
# 실제 값과 예측 값의 차이 계산
차이 = new_dC.values - dC[:actual_length]  # 실제 값의 길이에 맞게 dC를 슬라이싱
...
# 평균 절대 백분율 오차 (MAPE)
MAPE = np.mean(np.abs(차이 / new_dC.values)) * 100
print(f"평균 절대 백분율 오차 (MAPE): {MAPE:.2f}%")

# 마지막 해의 오차율 계산 (2023년)
실제값 = new_dC.iloc[-1]
예측값 = dC[actual_length - 1]  # 인덱스 맞춤
오차율 = (예측값 - 실제값) / 실제값 * 100
오차 = 예측값 - 실제값
print(f"2023년 오차: {오차:.0f}명")
